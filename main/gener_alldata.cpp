...
std::vector<std::pair<Eigen::Vector4d, Eigen::Vector4d>> CreatePointsLines(
...
int main() {
  ...
  // IMU model
  Param params;        // set Rbc & tbc
  IMU imuGen(params);  // acc_bias & gyro_bias set to zero
  // imu data include p v q ba bg
  // MotionData也是 p v q ba ba 外加上　acc & gyro readings
  // create imu data
  // imu pose gyro acc
  std::vector<MotionData> imudata;
  std::vector<MotionData> imudata_noise;
  for (float t = params.t_start; t < params.t_end;) {  // 20 seconds
  ...
  //预积分需要初始的速度和pose，即初始的p v q, 另外ba 和 bg初始化设置为zero了
  ...
  for (int n = 0; n < camdata.size(); ++n) {
    ...
    for (int i = 0; i < points.size(); ++i) {
      ...
      //            if( (obs(0)*460 + 255) < params.image_h && ( obs(0) * 460 +
      //            255) > 0 &&
      //                    (obs(1)*460 + 255) > 0 && ( obs(1)* 460 + 255) <
      //                    params.image_w )
      {
  ...
  for (int n = 0; n < camdata.size(); ++n) {
    ...
    //        Eigen::aligned_allocator<Eigen::Vector4d> > points_cam;    //
    //        ３维点在当前cam视野里
    std::vector<Eigen::Vector4d, Eigen::aligned_allocator<Eigen::Vector4d>>
        features_cam;  // 对应的２维图像坐标
